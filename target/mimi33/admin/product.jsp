...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            商品名称：<input name="pname" id="pname" value="${productVo.pName}">&nbsp;&nbsp;&nbsp;
            商品类型：<select name="typeid" id="typeid">
            <option value="-1">请选择</option>
            <c:forEach items="${typeList}" var="pt">
                <option value="${pt.typeId}"
                   <c:if test="${productVo.typeId==pt.typeId}">
                         selected="selected"
                   </c:if>
                  >${pt.typeName}</option>
            </c:forEach>
        </select>&nbsp;&nbsp;&nbsp;
            价格：<input name="lprice" id="lprice" value="${productVo.lprice}">-<input name="hprice" id="hprice" value="${productVo.hprice}">
...
<script type="text/javascript">
    ...
    function deleteBatch(page) {
        ...
        if(checkedLength==0){
            ...
            if(confirm("您确定删除"+checkedLength+"条数据吗？")){
                var pName=$("#pname").val()
    ...
    function del(pid,page) {
        if(confirm("您确定删除吗？")){
            ...
            $.ajax({
                url:"${pageContext.request.contextPath}/prod/deleteById.action",
    ...
    function one(pid,page) {
        ...
        var lprice=$("#lprice").val()
        var hprice=$("#hprice").val()
        var str = "pName="+pName + "&typeId="+typeId+"&lprice="+lprice+"&hprice="+hprice+"&pid="+pid+"&page="+page
        location.href="${pageContext.request.contextPath}/prod/updateProduct.action?"+str
...
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        var hprice=$("#hprice").val()
        $.ajax({
            "url":"${pageContext.request.contextPath}/prod/ajaxSplitPage.action",
            "data":{"pName":pName,"typeId":typeId,"lprice":lprice,"hprice":hprice,"page":page},
            "type":"post",
            "success":function (){
                $("#table").load("${pageContext.request.contextPath}/admin/product.jsp #table")
            }
